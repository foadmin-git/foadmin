...
<template>
  ...
  async function loadOptions(){
    const [orgRes, lvRes] = await Promise.all([orgTree(), listLevels()])
  ...
  const allRoles = ref([]); const selectedRoles = ref([]); const currentUserId = ref(null)
  async function openRoles(row){
    currentUserId.value = row.id
    const { all, selected } = await getUserRoles(row.id)
    allRoles.value = all; selectedRoles.value = selected
