        ...
        <el-dropdown trigger="hover">
          <span class="cursor-pointer block focus:outline-none focus:ring-0">
            <div class="rounded-full p-1 hover:bg-gray-200 transition-colors">
              <el-avatar :size="36" :src="avatarSrc" class="[&>img]:!border-none !border-none" />
            </div>
          </span>
          <template #dropdown>
...
const secondLevel = computed(() => {
  ...
  return level2.map((sec) => {
    ...
    const childrenTree = buildChildren(secKey)
    const flatChildren = flattenTree(childrenTree, 0)
...
function isPinned(item) { return pinned.value.some((p) => p.path === item.path) }
...
// 动态头像地址：优先使用 SHA256，其次自定义 URL，否则用默认头像
const avatarSrc = computed(() => {
  const u = userStore.user
  // 优先使用 SHA256 防止 ID 遍历攻击
  if (u?.avatar_sha256) {
    try { return mediaFiles.previewUrl(u.avatar_sha256) } catch { /* ignore */ }
  }
  // 兼容旧数据：如果没有 SHA256 但有 file_id
  if (u?.avatar_file_id && !u?.avatar_sha256) {
    try { return mediaFiles.previewUrl(u.avatar_file_id) } catch { /* ignore */ }
  }
  if (u?.avatar_url) return u.avatar_url
  return avatars
})

function goProfile() { router.push('/account/profile') }
function logout() {
  try { localStorage.removeItem('foadmin_pinned_v1') } catch {}
  userStore.clear()
  appStore.routesReady = false
